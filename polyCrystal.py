...
def periodicHexagonalCrystal(h=20, r=(2, 2), tol=1E-5, verbose=False):
    # Get the generators
    gens = rotatedCrystal(numpy.eye(3), size=(r[0], r[1], 1), a=h, cType='tr-or')
    tr = periodicVoronoiCell(L=numpy.diag(gens.cell)[:2], x0=gens.positions[:, 0], y0=gens.positions[:, 1])
    tr_rel = cvtRelax(tr, verbose=verbose, tol=tol)
    cr = tr2gr(tr_rel)
...
def cvtRelax(gr, **kwargs):
    ...
    pad_p = numpy.tile(pad.T, len(tvecs)).T + tvecs.repeat(len(pad),
                                                           axis=0)  # Periodic immutable points, without the central
    # peice
    # cut area outside the strip
    pad_p = pad_p[
        numpy.logical_and(*[numpy.logical_and(pad_p[:, i] > -sw, pad_p[:, i] <= gr.cell[i, i] + sw) for i in range(2)])]
    ...
    while maxUpd > tol and itNum < maxIter:
        ...
        if verbose and itNum % 10 == 0:
            print (itNum, maxUpd)
        # Make periodic
        bdry_p = numpy.tile(bdry.T, len(tvecs)).T + tvecs.repeat(len(bdry),
                                                                 axis=0)  # Periodic mutable points, without the
        # central peice
        bdry_p = bdry_p[numpy.logical_and(
            *[numpy.logical_and(bdry_p[:, i] > -sw, bdry_p[:, i] <= gr.cell[i, i] + sw) for i in range(2)])]
        ...
        _cPolyUtils.vorCentroid(len(Z) - len(bdry), len(Z), vor.regions, vor.point_region, vor.points[:, 0],
                                vor.points[:, 1], vor.vertices[:, 0], vor.vertices[:, 1], ox, oy)
    ...
    for reg in vor.regions:
        if len(reg) > 0 and -1 not in reg:  # Non-empty and non-open region
            ...
            if len(r1) > 0:
                ...
                for i, r1x, r1y, r2x, r2y in zip(range(len(r1)), vor.vertices[r1][:, 0], vor.vertices[r1][:, 1],
                                                 vor.vertices[r2][:, 0], vor.vertices[r2][:, 1]):
                    if r1x > r2x:
                        flag[i] = r1[i]
                    elif r2x > r1x:
                        flag[i] = r2[i]
...
def tr2gr(tr, periodic=True, num=[6, 6, 6], cutoff=0.1, vorMet=True):
    ...
    # Right boundary
    boundary[
        numpy.logical_and(numpy.logical_and(px < lx + ds, px > lx - ds), numpy.logical_and(py > -ds, py < ly + ds))] = 1
    # Bottom boundary
    boundary[numpy.logical_and(numpy.logical_and(py < ds, py > -ds), numpy.logical_and(px > -ds, px < lx + ds))] = 1
    # Top boundary
    boundary[
        numpy.logical_and(numpy.logical_and(py < ly + ds, py > ly - ds), numpy.logical_and(px > -ds, px < lx + ds))] = 1
